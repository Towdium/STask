...
\abstract{
  This project aims to develop an application to teach task graph scheduling. The application includes tutorials for concepts and algorithms in this area, in addition to the ability to simulate the execution of several communication models for arbitrary clusters and task graphs. The development of the application is based on Java and OpenGL (LWJGL). This report will provide detailed description of execution logic for communication models used in this project, with explanation of design and implementation. According to user testing, it shows most students can benefit from this application when learning task graph scheduling, but the learning experience can be further improved by adding more examples and challenges. Finally, more suggestions are given for future work.
}

\maketitle

\section*{Acknowledgements}
I would like to first thank Professor Murray Cole for his kind help and wise suggestions throughout this project. Secondly I would like to thank all the testers, who I cannot mention the names for anonymity, for their passionate participation and precious time. Finally I would also like to thank developers of LWJGL, an open source Java library that provides convenient accessability to many native resources, which makes it much easier to create games in Java.

\section*{Declaration}

I have read and understood the University's plagiarism guidelines.
...
\noindent\fbox{%
  ...
  In order to better understand this application, it is recommended that the reader refer to the online video that provides recording of all the interactions and levels, which is available at \url{https://youtu.be/IfGJpxUXV90}.
  }%
}
